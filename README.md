...
> **현황 (2026-04-22)**: 상용화 1.0 Phase A~F 코드 완료 + 관리자 권한 모델(USER_TP 기반) + 호텔 선택 플로우 완료. 심사 2026-05-20.

### 남은 것 (심사 전)
1. **하위 관리자 권한 부여 UI** — SYS_ADMIN 이 PROP/CMPX_ADMIN 에게 메뉴별 권한 할당하는 화면 (`INV.CCS_ROLE_GRANT` 테이블 + 관리 뷰)
2. **`DaolPmsSyncAdapter` 실 구현** — 현재 `syncLostFound/Voc/Rental/Duty` 는 로그만. 상용 배포 전 PMS 테이블 INSERT 작성
3. **`CCS_REPORT_DAILY` 롤업 스케줄러** — 현재는 실시간 GROUP BY. 트래픽 증가 시 배치 추가
4. **AuditLog AOP 전환** — Rental/Duty 는 아직 inline 훅 미적용
5. **카카오 REST API 키** → `NEARBY_PROVIDER=kakao`
6. **배포** — OCI Ampere 또는 Hetzner CX22 + Docker compose + duckdns
7. **심사 리허설** — `docs/DEMO_SCRIPT.md` 90초 시나리오, QR 태블릿 + 노트북 + 휴대폰 3기기
