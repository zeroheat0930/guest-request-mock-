...
> ## 🚨 브랜치 운영 규칙 — 무조건 `main` 직진 (2026-04-28~)
>
> **회사에서 작업하건, 집에서 맥북으로 작업하건, 모든 커밋·푸쉬는 `main` 브랜치에 직접 한다.**
>
> - feat/* 사이드 브랜치 만들지 말 것 — `feat/phases-b-f` 에 33+ 커밋이 갇혀 GitHub Pages 가 갱신 안 되던 문제로 결정됨
> - 1인 프로젝트라 PR/리뷰 단계 불필요. 머지 비용 0
> - GitHub Pages 가 `main` 에서 서빙 — landing 페이지를 즉시 반영하려면 main 직진 필수
> - 작업 시작 전: `git checkout main && git pull origin main`
> - 작업 종료 후: `git push origin main` (push 안 하고 자리 뜨지 말 것)
...
**`AiTranslateService` 백엔드**
- 모델 `claude-haiku-4-5-20251001` (config: `anthropic.translate.model`) — 짧은 통역에 최적
- 입력: `{text, targetLang, sourceLang?, tone?}` — sourceLang 기본 `auto`
- 4개국어 지원 (ko/en/ja/zh) → 12 페어 통역 + 자동 언어 감지
- system prompt 가 호텔 응대 톤(`polite_hotel`) 강제 — 정중한 호칭/존댓말 유지
- 출력은 번역문만 (인용/라벨 제거)

**`POST /api/ai/translate` + `/api/ai/status.translateEnabled`**

**`StaffTranslatorWidget.vue`**
- 우측 하단 플로팅 🌐 버튼, 클릭 시 380px 패널 오픈
- 4개 프리셋 토글: `게스트 → 한국어 / 한국어 → English / 한국어 → 日本語 / 한국어 → 简体中文`
- 텍스트 영역 + 타깃 언어 선택 + 번역 실행
- 결과 카드에 깃발/언어 페어 + 모델명 + 📋 클립보드 복사
- StaffShell 에 박혀 어느 스태프 페이지에서도 동일 위젯 즉시 호출

**시연 데모 임팩트**
- 일본인 게스트가 "シャワーが冷たいです" → 스태프가 위젯에 붙여넣기 → 한국어 "샤워 물이 차갑습니다" 즉시 확인
- 답변 한국어 "지금 바로 점검 보내드리겠습니다" → "ただいま点検にお伺いいたします" 정중한 응대 톤 유지
- 4개국어 X 4개국어 12 페어 + 자동 감지 — 호텔 다국어 운영 핵심
- 경쟁 트랙(통화 STT) 대비 우리는 **비동기 메시지 통역** — 노이즈/지연 0

**검증**
- `mvn -q -o compile` 클린
- `npx vite build` 클린, staff 번들 75 → 78kB (위젯 추가 정상)
